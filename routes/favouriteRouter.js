...
.get(cors.cors, authenticate.verifyUser, (req, res, next) => {
    ...
    .then((favroite) => {
        if(favroite) {
            ...
            var err = new Error('User have not favourites!!');
            err.status = 404;
            return next(err);
        }
    }, err => next(err))
...
.post(cors.corsWithOptions, authenticate.verifyUser, (req,res,next) => {
    ...
    .then((favroite) => {
        var user;
        if(favroite)
            user = favroite.filter(fav => fav.user._id.toString() === req.user.id.toString())[0];
        if(!user) 
            user = new Favorite({user: req.user.id});
        ...
        .then((userfav) => {
            ...
            res.setHeader('Content-Type','application/json');
            res.json(userfav);
            console.log('Fav created for user');
        }, (err) => next(err))
        .catch((err) => next(err))
    }, (err) => next(err))
    .catch((err)=>next(err));
